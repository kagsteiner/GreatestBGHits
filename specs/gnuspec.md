...
Please create an endpoint "analyzePositionFromMatch" that takes a GNU Backgammon match ID and position ID and returns a list of possible moves in this position - move in backgammon notation + equity after move.
...
I am aware of this approach requiring to start Gnu Backgammon for every single move analysis. But let's first cover this and then improve later. Remember, the match id + position ID describe the current position and the match completely.
